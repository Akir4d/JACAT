...
class Migration extends MY_Model
{
  ...
  public function upgrade_migration_restapi($drop = false)
  {
    if ($drop) {
      $this->load->dbforge();
      $this->dbforge->drop_table('api_access');
      $this->dbforge->drop_table('api_keys');
      $this->dbforge->drop_table('api_limits');
  ...
  public function upgrade_migration_ion($variant, $drop = false, $fake_users = array())
  {
    ...
    if ($drop) {
      $this->load->dbforge();
      $this->dbforge->drop_table($dbprefix . 'users_groups');
      $this->dbforge->drop_table($dbprefix . 'users');
      $this->dbforge->drop_table($dbprefix . 'login_attempts');
      $this->dbforge->drop_table($dbprefix . 'groups');
    }
