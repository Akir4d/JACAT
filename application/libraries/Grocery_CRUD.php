...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= "<div id='loading-$unique' style='display:none'><span id='upload-state-message-$unique'></span> <span class='spinner-border text-secondary'></span> <span id='progress-$unique'></span></div>";
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected function _is_video_file($value)
	{
		if (empty($value)) return false;
	...
	protected function _is_audio_file($value)
	{
		if (empty($value)) return false;
